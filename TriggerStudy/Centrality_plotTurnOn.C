...
void Centrality_plotTurnOn(){
    const std::string recFilePath = "/Users/soohwanlee/RunPreparation/store/Forest_HIMinimumBias2_run327237_merged.root";
  const std::vector<std::pair<std::string, std::string> > hltFilePath = {
      {"Online", "/Users/soohwanlee/RunPreparation/store/OpenHLTTree_HIMinBias_103X_dataRun2_HLT_Run327237.root"},
      {"Miscalibrated", "/Users/soohwanlee/RunPreparation/store/OpenHLTTree_HIMinBias_103X_dataRun2_HLT_ForHITestsV3_v2_Run327237.root"}
  };
  ...
  for (const auto& p: TRIGLIST){
    for (const auto& t : p.second) {
      size_t centf = t.rfind("Centrality");
      if (centf!= std::string::npos){
	for (const auto& h : hiBin) {
	  const std::string name = "eff"+p.first+"_"+t+"_"+h.first;
	  effMapTitle[p.first][t][h.first] = name;
//	  std::cout << name << std::endl;
          for (const auto& c : hltFilePath) {
  	    effVec.emplace_back(TEfficiency("", "", h.second.size()-1, h.second.data()));
	    effVec.back().SetName((name+"_"+c.first).c_str());
	    effMapIdx[p.first][t][h.first][c.first] = effVec.size()-1;
//	    std::cout << t << std::endl;
 	  }
        }
      }
    ...
    for (const auto& t : TRIGLIST) {
      if (t.first.rfind("Muon"    )!=std::string::npos) { doParticle.insert("muon");     }
  ...
  // prepare multi-threading
    ...
    auto fillEfficiency = [=](int idx)
    {
	  ...
	  const auto& muons = particles[iPar1];
          // extract variables
          const auto var = std::map<std::string, double>({ {"Pt", muons.first.Pt()},{"Eta", muons.first.Eta()} });
	  // loop over single muon triggers
	      ...
	      size_t centf = path.rfind("Centrality");
	      if (centf!= std::string::npos){

  	        // check if trigger is matced 
  	        const auto isMatched = triggerInfo.isTriggerMatched(muons.first, path);
  	        // fill efficiency
	        for( const auto& h : hiBin ) {
  	          const auto& index = effMapIdx.at(parTag).at(path).at(h.first).at(hltFileP.first);
  	          effV[index].Fill(isMatched, bObj);
  	          idxS.insert(index);
	        }
  	      }
            }
	  }
          // loop over double muons

          parTag = "Double"+parName;
          if (TRIGLIST.find(parTag)!=TRIGLIST.end()) {

	    for (size_t iPar2=iPar1+1; iPar2<particles.size(); iPar2++){
              const auto& dmuons = particles[1];
              const auto p4 = muons.first + dmuons.first;
    	      // extract variables
  	      const auto varDP = std::map<std::string, double>({ {"Pt", p4.Pt()},{"Rapiditiy", p4.Rapidity()} });
  	      // loop over double particle triggers
  	      for (const auto& path : TRIGLIST.at(parTag)) {
	      size_t centf = path.rfind("Centrality");
	      if (centf!= std::string::npos){
  	          // check if trigger is matched
  	          const bool isMatched1 = triggerInfo.isTriggerMatched(muons.first, path);
  	          const bool isMatched2 = triggerInfo.isTriggerMatched(dmuons.first, path);
  ...
  TFile *file = new TFile("outputEff_Centrality_HLT_ECAL_track_miscal.root","recreate");
  ...
  for (const auto& p : effMapIdx) {
	...
	std::cout << name << std::endl;
        ...
        for (auto& g : graphM) {
          ...
          g.second.Draw("samep");
        g.second.SetName(Form("%s_miscal_Cent",name.c_str()));
        g.second.Write();
        }
	...
	// draw legend
        ...
        const std::string plotDir = "Cent_Plot/" + par;
        makeDir(plotDir + "/png/");
        makeDir(plotDir + "/pdf/");
        // Save Canvas
