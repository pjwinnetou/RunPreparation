...
using namespace std;
bool MBTrigFire(vector<int> trigInt);
void makeRatePlot(const char* filename = "openHLT",int isel=0)
{
  ...
  for(const auto& triggerStr : trigname){
    hist[trigNum] = new TH1D(Form("h_%s",triggerStr.c_str()),";;;",4,0,4);
    trigNum++;
  ...
  for(int itrig=0;itrig<nTrig;itrig++){
    ...
    tree->GetEntry(ievt);
      if(trig[itrig]==1) hist[itrig]->Fill(0);
      if(MBTrigFire(trig)){ 
        nMBevt++;
        if(trig[itrig]==1) hist[itrig] -> Fill(1);
      }
    }
    if(hist[itrig]->GetBinContent(1)!=0){
      hist[itrig]->SetBinContent(3,(double)hist[itrig]->GetBinContent(1)/tree->GetEntries());
      if(hist[itrig]->GetBinContent(2)!=0) hist[itrig]->SetBinContent(4,(double)hist[itrig]->GetBinContent(2)/nMBevt);
...
bool MBTrigFire(vector<int> trigInt){
  return trig[MBTrigline1] ==1 || trig[MBTrigline2] ==1;
}
